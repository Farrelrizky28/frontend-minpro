...
import { getEmployeeResponse, reqUpdateCurriculum, resCreateCurriculum, resCreateSection, resCreateSectionDetail, resDeleteCurri, resGetCurrNum, resGetCurriculum, resGetIdCurriculum, resGetSectionMergeRes, resUpdateCurriculum } from '../action/actionReducer'
...
export function* handleGetSectionMerge(): any {
      ...
      export function* handleDeleteCurri(action: any): any {
        try {
          const result = yield call(apiMethod.deleteCurriculum, action.payload);
          console.log(result);
          yield put(resDeleteCurri(result.data));
        } catch (error) {
          yield put(resDeleteCurri({ message: error, status: 400 }));
        }
      }
