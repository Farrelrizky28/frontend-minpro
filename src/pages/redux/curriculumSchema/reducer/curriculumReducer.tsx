...
function curriculumReducer(state = initialState, action: any) {
    ...
    switch (type) {
       ...
       case ActionType.RES_GET_CURRICULUM_BY_ID:
            return { ...state, entity_id: payload, status: payload.status, message: payload.message, refresh: true }
    
        case ActionType.RES_GET_CURRNUM:
            return { ...state, currnum: currnumber, status: currnumber.status, message: currnumber.message, refresh: true }
        case ActionType.RES_CREATE_CURRICULUM:
            return { state,status: payload.status, message: payload.message, refresh: false }
        ...
        case ActionType.RES_UPDATE_CURRICULUM:
           ...
           case ActionType.RES_GET_MERGE:
            return { state, section:sections,message:sections.message,status: sections.status, refresh: true }
        
        case ActionType.GETEMPLOYEE_RESPONSE:
            return { ...state, userEmployee: payload, refresh: true }
        
         case ActionType.RES_DELETE_CURRICULUM:
