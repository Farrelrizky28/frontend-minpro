import { reqGetCurriclum } from "@/pages/redux/curriculumSchema/action/actionReducer"
...
import { Box, Menu, IconButton, MenuItem, styled, MenuProps, alpha } from "@mui/material"
import { MoreVert as MoreVertIcon, Edit as EditIcon, Delete as DeleteIcon, Router, MoreVertRounded, DoDisturbOnOutlined, Edit, DeleteOutlineRounded, PlayArrowOutlined, GradingRounded } from '@mui/icons-material'
import Link from 'next/link'
import Pagination from "./pagination"
import deleteCurr from "./deleteCurriculum"
import DeleteCurr from "./coba/delete"
...
const Curriculum = () => {
    ...
    const handleDeleteCurriculum = () => {
        ...
        return (
            <>
                {/* {isDelete ? <deleteCurr show={isDelete} postById={postById} closeModal={() => setIsDelete(false)} /> : ''} */}
                <Content1
                    title={`Curriculum`}
                ...
                <div className=" bg-green-600 w-full p-4 text-center">
                    ...
                    <select className="order-0 w-24  ml-2 inline-flex"
                        value={selectedValue}
                        onChange={(e) => { setSearchFilter(e.target.value); setSelectedValue(e.target.value) }}>
                        <option value="">Status</option>
                        ...
                        <tbody>
                            {(currentItems || []).map((data: any, i: any) => (
                                <tr>
                                    ...
                                    <td className="px-6 py-3 text-sm text-gray-500 text-center">{data.prog_title}</td>
                                    <td className="px-6 py-3 text-sm text-gray-500 text-center">{data.prog_headline}</td>
                                    <td className="px-6 py-3 text-sm text-gray-500 text-center">{data.prog_duration} {data.prog_duration_type}</td>
                                    ...
                                    <td className="px-6 py-3 text-sm text-gray-500 text-center">
                                        ...
                                        <div> {data.total_batch}Batch </div></td>
                                    <td className="px-6 py-3 text-sm text-gray-500 text-center">{data.prog_learning_type}</td>
                                    <td className="px-6 py-3 text-sm text-gray-500 text-center">
                                        ...
                                        >
                                            ...
                                            <Menu.Item>
                                                {({ active }) => (
                                                    <button
                                                        onClick={() => {
                                                            setPostById(data.prog_entity_id);
                                                            setIsDelete(true);
                                                        }}
                                                        className={`${active
