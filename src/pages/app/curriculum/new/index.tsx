...
import { AccordionDetails, AccordionSummary, Autocomplete, Button, Card, Dialog, DialogContent, DialogTitle, FormControl, Grid, Icon, IconButton, InputLabel, Menu, MenuItem, Select, SelectChangeEvent, TextField, duration, Typography, DialogProps } from "@mui/material";
...
import { getEmployee, reqCreateCurriculum, reqCreateSection, reqGetCurrNum, reqGetCurriclum, reqGetSectionMerge } from '../../../redux/curriculumSchema/action/actionReducer';
...
const curriculum = () => {
    ...
    useEffect(() => {
        const nameImage = userEmployee?.find((item) => item.emp_entity_id === selectedId)
    ...
    let { master, refresh } = useSelector(
        (state: any) => state.masterReducer
    )
    ...
    const [openedSectionIndex, setOpenedSectionIndex] = useState<number | null>(null);
    ...
    return (
        <Content1 title="CREATE CURICULUM" fungsi1={() => router.back()} namafungsi1="BACK">
            <div>
                    ...
                    <div className="lg:grid lg:grid-cols2">
                        <section className="my-8">
                            ...
                            {section?.map((item: any, index: any) => (
                                ...
                                >
                                    <AccordionSummary expandIcon={<KeyboardArrowDownIcon />} onClick={() => handleOpen(index)}>
                                        <div className="flex justify-between">
                                            <div>{item.sect_title}</div>
                                            <div className="absolute top-4 right-20 text-lg">
                                                {!item.sect_total_minute ? '0 Minutes' : item.sect_total_minute >= 60
